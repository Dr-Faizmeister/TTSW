...
var
  ...
  rog: array of Real48; // gas density
  ro: array of Real48; // summary fluid density
  viscg: array of Real48; // gas viscosity
  visc: array of Real48; // summary fluid viscosity
  ...
  sQ: array of Real48; // summary specific rate
  sQo: array of Real48; // specific rate (oil)
  sQg: array of Real48; // specific rate (gas)
  sQw: array of Real48; // specific rate (water)
  inQ: array[0..20] of Real48; // summary rate
  inQo: array[0..20] of Real48; // oil rate
  inQg: array[0..20] of Real48; // gas rate
  inQw: array[0..20] of Real48; // water rate
  inro: array[0..20] of Real48; // summary inflow density
  ...
  c, co, cg, cw, fr, Re, lambda, lambdao, lambdag, lambdaw, lambda_for, lambda_cem, ro_for, c_for, Pr, Nu,
  alpha_for, h_fluid, h_cem, h_for, T_d, time_d, tau, R0, depth, depth0, dl0, eo, eg, ew,
  p0, hbot, htop, inQ0, TG, TG0, inT0, T0, n0, Qw, fluid, yo, yg, yw, roo, row, visco, viscw : Real48;
...
begin
  rog[i]:=p[i]*M/Z[i]/Rg/T[i-1];
  ro[i]:=roo*yo+rog[i]*yg+row*yw;
  ...
  if (T[i]/T_c < 1) then viscg[i]:=(critical_visc*power(T[i]/T_c, 0.965))/1000000;
  if (T[i]/T_c > 1) then viscg[i]:=(critical_visc*power(T[i]/T_c, 0.71+0.29*T_c/T[i]))/1000000;
  visc[i]:=visco*yo+viscg[i]*yg+viscw*yw;

  lambda:=lambdao*yo+lambdag*yg+lambdaw*yw;
end;
...
// -------- holdup calculation ---------------------------------------------- //
procedure holdup(i, k: Integer);
begin
  sQo[i]:=sQo[i-1]+inQo[k]/((L_top[k]-L_bot[k])/dl);
  sQg[i]:=sQg[i-1]+inQg[k]/((L_top[k]-L_bot[k])/dl);
  sQw[i]:=sQw[i-1]+inQw[k]/((L_top[k]-L_bot[k])/dl);
  inQ[k]:=inQo[k]+inQg[k]+inQw[k];
  sQ[i]:=sQ[i-1]+inQ[1]/((L_top[k]-L_bot[k])/dl);
  yo:=sQo[i]/sQ[i];
  yg:=sQg[i]/sQ[i];
  yw:=sQw[i]/sQ[i];
end;
...
// ------ initial conditions------------------------------------------------- //
  ...
  Z[0]:=0.92874;
  roo:=780; // initial density (gas)
  row:=1000;
  rog[0]:=p[0]*M/Z[0]/Rg/T[0]; // initial density (gas)
  visco:=0.002; // initial viscosity (oil)
  viscw:=0.0005; // initial viscosity (water)
  viscg[0]:=1.19e-05; // initial viscosity (gas)
  co:=2100; // thermal capacity (oil)
  cg:=2500; // thermal capacity (gas)
  cw:=4200; // thermal capacity (water)
  eo:=0.0000004; // J-T (oil)
  eg:=-0.000003; // J-T (gas)
  ew:=0.000000235; // J-T (gas)
  kappa:=0.0150; // piezoconductivity
  tM:=500;  // measuring time
  lambdao:=0.136; // thermal conductivity (oil)
  lambdag:=0.1; // thermal conductivity (gas)
  lambdaw:=0.65; // thermal conductivity (water)
